...
class Marketplace {
  ...
  constructor(network, address, storage) {
    ...
    this.roles = new thirdwebCheckout.ContractRoles(this.contractWrapper, Marketplace.contractRoles);
    ...
    this.direct = new thirdwebCheckout.MarketplaceDirect(this.contractWrapper, this.storage);
    this.auction = new thirdwebCheckout.MarketplaceAuction(this.contractWrapper, this.storage);
  ...
  async getListing(listingId) {
    ...
    if (listing.assetContract === ethers.constants.AddressZero) {
      throw new thirdwebCheckout.ListingNotFoundError(this.getAddress(), listingId.toString());
    }
    switch (listing.listingType) {
      case thirdwebCheckout.ListingType.Auction:
        {
          return await this.auction.mapListing(listing);
        }
      case thirdwebCheckout.ListingType.Direct:
        {
  ...
  async getActiveListings(filter) {
    ...
    return filtered.filter(l => {
      return l.type === thirdwebCheckout.ListingType.Auction && ethers.BigNumber.from(l.endTimeInEpochSeconds).gt(now) && ethers.BigNumber.from(l.startTimeInEpochSeconds).lte(now) || l.type === thirdwebCheckout.ListingType.Direct && l.quantity > 0;
    });
  }
  ...
  async isRestrictedToListerRoleOnly() {
    const anyoneCanList = await this.contractWrapper.readContract.hasRole(thirdwebCheckout.getRoleHash("lister"), ethers.constants.AddressZero);
  ...
  async getOffers(listingId) {
    ...
    return await Promise.all(listingEvents.map(async e => {
      return await thirdwebCheckout.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), {
      ...
      case thirdwebCheckout.ListingType.Direct:
        {
          ...
          return await this.direct.makeOffer(listingId, quantity, thirdwebCheckout.isNativeToken(listing.currency) ? thirdwebCheckout.NATIVE_TOKENS[chainId].wrapped.address : listing.currency, pricePerToken);
        }
      case thirdwebCheckout.ListingType.Auction:
        {
  ...
  async allowListingFromSpecificAssetOnly(contractAddress) {
    ...
    if (members.includes(ethers.constants.AddressZero)) {
      encoded.push(this.encoder.encode("revokeRole", [thirdwebCheckout.getRoleHash("asset"), ethers.constants.AddressZero]));
    }
    encoded.push(this.encoder.encode("grantRole", [thirdwebCheckout.getRoleHash("asset"), contractAddress]));
  ...
  async allowListingFromAnyAsset() {
    ...
    for (const addr in members) {
      encoded.push(this.encoder.encode("revokeRole", [thirdwebCheckout.getRoleHash("asset"), addr]));
    }
    encoded.push(this.encoder.encode("grantRole", [thirdwebCheckout.getRoleHash("asset"), ethers.constants.AddressZero]));
    await this.contractWrapper.multiCall(encoded);
  }
  ...
  async getAllListingsNoFilter(filterInvalidListings) {
    const listings = await Promise.all(Array.from(Array((await this.contractWrapper.readContract.totalListings()).toNumber()).keys()).map(async i => {
      ...
      try {
        ...
        if (err instanceof thirdwebCheckout.ListingNotFoundError) {
          return undefined;
        } else {
      ...
      if (listing.type === thirdwebCheckout.ListingType.Auction) {
        return listing;
      }
