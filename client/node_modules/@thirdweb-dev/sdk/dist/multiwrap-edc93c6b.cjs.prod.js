...
require('zod');
require('./QueryParams-d34a661c.cjs.prod.js');
require('bn.js');
 ...
 * @beta
 */
class Multiwrap extends thirdwebCheckout.StandardErc721 {
  /**
   * Configure royalties
  ...
  constructor(network, address, storage) {
    ...
    this.abi = abi;
    this.metadata = new thirdwebCheckout.ContractMetadata(this.contractWrapper, thirdwebCheckout.MultiwrapContractSchema, this.storage);
    this.roles = new thirdwebCheckout.ContractRoles(this.contractWrapper, Multiwrap.contractRoles);
  ...
  async getWrappedContents(wrappedTokenId) {
    ...
    for (const token of wrappedTokens) {
      switch (token.tokenType) {
        case 0:
          {
            const tokenMetadata = await thirdwebCheckout.fetchCurrencyMetadata(this.contractWrapper.getProvider(), token.assetContract);
            erc20Tokens.push({
  ...
  async wrap(contents, wrappedTokenMetadata, recipientAddress) {
    const uri = await thirdwebCheckout.uploadOrExtractURI(wrappedTokenMetadata, this.storage);
  ...
  async toTokenStructList(contents) {
    ...
    if (contents.erc20Tokens) {
      for (const erc20 of contents.erc20Tokens) {
        const normalizedQuantity = await thirdwebCheckout.normalizePriceValue(provider, erc20.quantity, erc20.contractAddress);
        const hasAllowance = await thirdwebCheckout.hasERC20Allowance(this.contractWrapper, erc20.contractAddress, normalizedQuantity);
        if (!hasAllowance) {
