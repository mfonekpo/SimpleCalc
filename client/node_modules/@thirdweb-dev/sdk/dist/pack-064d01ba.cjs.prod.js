...
var thirdwebCheckout = require('./thirdweb-checkout-5ae0767f.cjs.prod.js');
...
const CommonWrappableSchema = zod.z.object({
  contractAddress: thirdwebCheckout.AddressSchema
});
...
const ERC1155WrappableSchema = CommonWrappableSchema.extend({
  tokenId: thirdwebCheckout.BigNumberishSchema,
  quantity: thirdwebCheckout.BigNumberishSchema
});
...
const ERC1155RewardSchema = ERC1155WrappableSchema.omit({
  ...
  quantityPerReward: thirdwebCheckout.BigNumberishSchema
});
...
const ERC20RewardContentsSchema = ERC20RewardSchema.extend({
  totalRewards: thirdwebCheckout.BigNumberishSchema.default("1")
});
...
const PackMetadataInputSchema = PackRewardsOutputSchema.extend({
  ...
  rewardsPerPack: thirdwebCheckout.BigNumberishSchema.default("1"),
  openStartTime: thirdwebCheckout.RawDateSchema.default(new Date())
});
 ...
 * @public
 */
class Pack extends thirdwebCheckout.StandardErc1155 {
  /**
   * Configure royalties
  ...
  constructor(network, address, storage) {
    ...
    let contractWrapper = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : new thirdwebCheckout.ContractWrapper(network, address, abi, options.gasless && "openzeppelin" in options.gasless ? {
      ...options,
      gasless: {
        openzeppelin: {
    ...
    this.metadata = new thirdwebCheckout.ContractMetadata(this.contractWrapper, thirdwebCheckout.PackContractSchema, this.storage);
    this.roles = new thirdwebCheckout.ContractRoles(this.contractWrapper, Pack.contractRoles);
  ...
  async isTransferRestricted() {
    const anyoneCanTransfer = await this.contractWrapper.readContract.hasRole(thirdwebCheckout.getRoleHash("transfer"), ethers.ethers.constants.AddressZero);
  ...
  async getPackContents(packId) {
    ...
    for (let i = 0; i < contents.length; i++) {
      ...
      switch (reward.tokenType) {
        case 0:
          {
            const tokenMetadata = await thirdwebCheckout.fetchCurrencyMetadata(this.contractWrapper.getProvider(), reward.assetContract);
  ...
  async createTo(to, metadataWithRewards) {
    const uri = await thirdwebCheckout.uploadOrExtractURI(metadataWithRewards.packMetadata, this.storage);
  ...
  async toPackContentArgs(metadataWithRewards) {
    ...
    for (const erc20 of erc20Rewards) {
      const normalizedQuantity = await thirdwebCheckout.normalizePriceValue(provider, erc20.quantityPerReward, erc20.contractAddress);
      ...
      const hasAllowance = await thirdwebCheckout.hasERC20Allowance(this.contractWrapper, erc20.contractAddress, totalQuantity);
      if (!hasAllowance) {
    ...
    for (const erc721 of erc721Rewards) {
      const isApproved = await thirdwebCheckout.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc721.contractAddress, erc721.tokenId, owner);
      if (!isApproved) {
    ...
    for (const erc1155 of erc1155Rewards) {
      const isApproved = await thirdwebCheckout.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc1155.contractAddress, erc1155.tokenId, owner);
      if (!isApproved) {
